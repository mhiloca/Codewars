...
def sum_of_intervals(intervals):
    return len(list(reduce(lambda z, y: z | y, map(lambda a: {x for x in range(a[0], a[1])}, intervals))))


def sum_of_intervals_v2(intervals):
    result = {}
    for start, stop in intervals:
        result = result | {x for x in range(start, stop)}
    return len(result)


array = [[1, 5], [10, 20], [1, 6], [16, 19], [5, 11]]
array2 = [[1, 2], [6, 10], [11, 15]]
array3 = [[1, 4], [7, 10], [3, 5]]
print(sum_of_intervals(array))
